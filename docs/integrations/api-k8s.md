...
This guide provides a step-by-step process for deploying API Sec Proxy in a Kubernetes environment, aimed at enhancing API
...
## 2. Deploying API Sec Proxy

Configure API Sec Proxy receiver by following [this](https://github.com/accuknox/SentryFlow/blob/main/docs/receivers.md). Then deploy updated API Sec Proxy manifest by following
`kubectl` command:
...
Then, check if API Sec Proxy is up and running by:
...
API Sec Proxy has now been deployed in the cluster. In addition, API Sec Proxy exports API access events through `gRPC`.

You can use `sfctl` the API Sec Proxy client to view or filter captured API access events
...
For more info check [this](https://github.com/AccuKnox/SentryFlow/blob/main/sfctl/Readme.md).
