...
## Prompt Firewall Rules for LLM Defense
...
**See Onboarding Steps:** [Onboard LLM Defense App](https://help.accuknox.com/use-cases/llm-defense-app-onboard/).

**Read Detailed Blog:** [Protecting LLMs with AccuKnox's Prompt Firewall](https://accuknox.com/blog/protecting-llms-with-accuknoxs-prompt-firewall/).
...
![alt text](./images/release-notes/v3.3/image.png)
...
Uses a Terraform-based onboarding flow where customers download a small Terraform bundle, fill in required variables, and run terraform apply to provision the scanner VM in their account. The onboarding page pre-fills variables and provides a tfvars download button. Resource requirements and configuration notes are displayed during setup.

![alt text](image-5.png)

### OpenShift (Private Cloud)
Customers deploy the AccuKnox Operator directly from the OpenShift console. Air-gapped clusters use special images provided for offline environments. The onboarding page supplies token creation, copy instructions, and environment-specific setup steps.

![alt text](./images/release-notes/v3.3/image-19.png)

### On-Prem or Unsupported Platforms (Agent-Based)
...
- **Non-air-gapped**: A single command (curl https://.../install.sh | sh -) installs the agent and pulls all required components from AccuKnox.
- **Air-gapped:** The same installer is used with OMNI_SKIP_DOWNLOAD=1, and the vulnerability database is pulled from the customer’s private registry or a local file path.
This path mirrors the standard onboarding flow and supports options like scan interval and malware-scanning setup.
